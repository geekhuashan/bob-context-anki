...
- 使用 MiniMax 生成英语发音，并直接保存到 Anki 媒体库。
...
5. 新卡的注释和发音会并行生成；MP3 成功写入 Anki 媒体库后，才会创建完整卡片。
6. 旧卡只补缺失内容：仅缺发音时不会重新生成注释，仅缺注释时也不会重新生成发音。
...
| 发音 API Key | 空 | 留空复用注释 Key，也可单独填写 MiniMax Key |
| 发音 API 地址 | `https://api.minimaxi.com` | MiniMax 根地址、`/v1` 或完整 `t2a_v2` 地址 |
| 发音模型 | `speech-2.8-hd` | 生成 MP3 单词发音 |
| 发音音色 | `English_Graceful_Lady` | 英语发音音色，语速固定为 0.85 |
| 最低词汇等级 | B2 及以上 | 控制候选词的最低 CEFR-J 等级 |
| 已认识词 | 空 | 排除已经掌握的词，支持空格、逗号和分号分隔 |

CEFR-J 1.6 只覆盖 A1 到 B2。未收录内容会标记为“词表外 / 专业词”，不会被冒充为 C1 或 C2。

注释服务使用 OpenAI 兼容的 `chat/completions` 接口。发音服务固定使用 MiniMax `t2a_v2`；同一套 MiniMax Key 可以同时用于两者，插件都通过标准的 `Authorization: Bearer` 请求头发送。
...
插件只在用户点击一个尚未收录的新词，或本插件此前创建的不完整卡片时，将完成缺失内容所需的数据发送到对应服务：注释服务接收目标词和原句，MiniMax 发音服务只接收目标词。生成的 MP3 以 base64 通过本机 `http://127.0.0.1:8765` 写入 Anki 媒体库。API Key 由 Bob 的安全设置保存，不包含在源码、构建产物或 Release 中。插件不读取其他 Bob 插件的数据。
