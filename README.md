...
4. AnkiConnect 先检查重复；只有新词或本插件创建的空注释卡才调用配置的注释服务。
5. 注释完整生成后，新词会一次性建卡，旧的空注释卡则只补全缺失内容。

如果只翻译句子、没有点击候选词，插件不会请求注释服务或写入 Anki。注释配置缺失或请求失败时也不会创建空白卡片。
...
MiniMax 用户可直接填写 Bob 中同一套 API Key，并保留默认地址与模型。其他 OpenAI 兼容服务可替换地址和模型。插件只使用标准的 `Authorization: Bearer` 请求头。

## 当前限制

- 牌组、笔记类型和字段结构目前固定，必须先按文档配置 Anki。
- 插件暂不生成音频，`Audio` 字段保持空白；可继续使用现有音频补全流程。
...
插件只在用户点击一个尚未收录的新词，或本插件此前创建的空注释卡时，将该词和原句发送到用户配置的 OpenAI 兼容注释服务；随后向本机 `http://127.0.0.1:8765` 写入完整词卡或补全旧卡。API Key 由 Bob 的安全设置保存，不包含在源码、构建产物或 Release 中。插件不读取其他 Bob 插件的数据。
